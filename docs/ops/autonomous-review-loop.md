...
Authority: [Canonical loop policy](../README.md#optional-execution-and-review-loop). This reference defines operational details, not a grant or a second status register. References below to active progress mean the authoritative docs/progress-agent.md; docs/progress.md is only its derived human summary. Adoption and examples leave the loop Off.
